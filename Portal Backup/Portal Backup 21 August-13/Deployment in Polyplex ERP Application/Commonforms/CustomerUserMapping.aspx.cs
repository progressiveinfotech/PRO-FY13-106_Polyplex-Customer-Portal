...
public partial class Sales_MasterFinalDestination : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                lbl.Text = "Customer User Mapping";
                ...
                txtUserName.Attributes.Add("style", "background:lightgray");
                txtCustomerName.Attributes.Add("readonly", "true");
                txtCustomerName.Attributes.Add("style", "background:lightgray");
                txtUserType .Attributes.Add("readonly", "true");
                txtUserType.Attributes.Add("style", "background:lightgray");
    ...
    protected void imgBtnUserCode_Click(object sender, ImageClickEventArgs e)
    {
        /// <summary>
        ...
        try
        {
            ...
            HidPopUpType.Value = "User";
            lPopUpHeader.Text = "User Master";
            lSearch.Text = "Search By User Code/Name/Type: ";
            FillAllUser("");
    ...
    {
        /// <summary>
        /// This event is used to validate entered usercode and populate user popup while entered usercode is not exist.
        /// </summary>
        try
        {
            string query = @"SELECT U.UserID,U.UserCode,U.UserName,Utype.Type from Com_User_Mst AS U Inner join Com_UserType_Mst Utype On
                             U.UserTypeId=Utype.UserTypeId WHERE U.ActiveStatus =1 AND Utype.Type in ('Customer','C_Owner','AR_Owner') and
                             U.UserCode = '" + txtUserCode.Text.Trim() + "'";
            ...
            if (dt.Rows.Count > 0)
            {
                HidUserId.Value = dt.Rows[0]["UserID"].ToString();
            ...
            else
            {
                ...
                {  
                    ...
                    HidPopUpType.Value = "User";
                    lPopUpHeader.Text = "User Master";
                    lSearch.Text = "Search By User Code/Name: ";
                    FillAllUser("");
                    ModalPopupExtender2.Show();
                }
                catch { }
    ...
    protected void imgBtnCustomerGroup_Click(object sender, ImageClickEventArgs e)
    {
        /// <summary>
        /// This event is used to populate all the customers in popup.
        /// </summary> 
        ...
        {
            txtSearchFromPopup.Text = "";
            HidPopUpType.Value = "CustomerGroup";
            lPopUpHeader.Text = "Customer Group Master";
            lSearch.Text = "Search By Customer Code/Name: ";
            FillAllCustomerGroup("");
            ModalPopupExtender2.Show();
        }
        catch { }
    }

    protected void txtCustomerGroup_TextChanged(object sender, EventArgs e)
    {
        /// <summary>
        /// This event is used to validate entered customercode and populate user popup while entered customercode is not exist.
        /// </summary>
        ...
        {
            string query = @"SELECT U.UserID,U.UserCode,U.UserName,Utype.Type from Com_User_Mst AS U Inner join Com_UserType_Mst Utype On
                             U.UserTypeId=Utype.UserTypeId WHERE U.ActiveStatus =1 AND Utype.Type in ('Customer') and
                             U.UserCode = '" + txtUserCode.Text.Trim() + "'";

            DataTable dt = new DataTable();
            dt = com.executeSqlQry(query);

            HidCustomerGroupId.Value = "";
            txtCustomerGroup.Text = "";

            if (dt.Rows.Count > 0)
            {
                HidCustomerGroupId.Value = dt.Rows[0]["UserID"].ToString();
                txtCustomerGroup.Text = dt.Rows[0]["UserName"].ToString();
            }
            else
            {
                try
    ...
    protected void gvPopUpGrid_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        try
        {
            ...
            if (e.CommandName == "select")
            {
                ...
                {
                    HidUserId.Value = gvPopUpGrid.Rows[gvPopUpGrid.SelectedIndex].Cells[1].Text;
                    txtUserCode.Text = gvPopUpGrid.Rows[gvPopUpGrid.SelectedIndex].Cells[2].Text;
                    txtUserName.Text = gvPopUpGrid.Rows[gvPopUpGrid.SelectedIndex].Cells[3].Text;
                    txtUserType.Text = gvPopUpGrid.Rows[gvPopUpGrid.SelectedIndex].Cells[4].Text;
                }
                ...
                {
                    HidCustomerGroupId.Value = gvPopUpGrid.Rows[gvPopUpGrid.SelectedIndex].Cells[1].Text;
                    txtCustomerGroup.Text = gvPopUpGrid.Rows[gvPopUpGrid.SelectedIndex].Cells[2].Text;
                }
            }
        }
        catch { }
    ...
    protected void ImgBtnSave_Click(object sender, ImageClickEventArgs e)
    {
        ...
        try
        {
            ...
            {
                DataTable dt = new DataTable();
                string query = @"select AutoId from tblCustomerUserMapping where UserId ='"+HidUserId.Value+"' and CustomerId ='"+HidCustomerId.Value+"'";
                dt = com.executeSqlQry(query);
                if (dt.Rows.Count > 0)
                {
            ...
            cmd.Parameters.Add("@UserId", SqlDbType.Int).Value = com.STRToInt(HidUserId.Value);
            cmd.Parameters.Add("@CustomerId", SqlDbType.Int).Value = com.STRToInt(HidCustomerId.Value);
            cmd.Parameters.Add("@CustomerGroupId", SqlDbType.Int).Value = com.STRToInt(HidCustomerGroupId.Value);
            ...
            cmd.CommandText = "SP_InsertUpdate_In_tblCustomerUserMapping";
            cmd.ExecuteNonQuery();
    ...
    protected void makeGrid()
    {
        ...
        try
        {
            string query = @"SELECT [AutoId]
                          ...
                          ,D.CustomerCode
                          ,D.Name
                          ,(select UserName from Com_User_Mst where UserID =A.CustomerGroupId) as CustomerGroup
                          ,A.[ActiveStatus]
                          FROM [tblCustomerUserMapping] as A inner join Com_User_Mst as B on A.UserId = B.UserID inner join Com_UserType_Mst
                          as C on C.UserTypeId =B.UserTypeId inner join Sal_Glb_Customer_Mst as D on D.CustomerID =A.CustomerId 
                          where (B.UserCode like '%" + txtSearch.Text.Trim() + "%' or B.UserName like '%" + txtSearch.Text.Trim() + "%' or";
                   query +=" C.Type like '%" + txtSearch.Text.Trim() + "%' or D.CustomerCode like '%" + txtSearch.Text.Trim() + "%' or D.Name";
                   query +=" like '%" + txtSearch.Text.Trim() + "%')  order by A.AutoId desc";
    ...
    private void BindHeaderRecords(string AutoId)
    {
        ...
        try
        {
            ...
            string query = @"SELECT [AutoId]
                          ...
                          ,(select UserName from Com_User_Mst where UserID =A.CustomerGroupId) as CustomerGroup
                          ,A.[ActiveStatus]
                      FROM [tblCustomerUserMapping] as A inner join Com_User_Mst as B on A.UserId = B.UserID inner join Com_UserType_Mst as C
                      on C.UserTypeId =B.UserTypeId inner join Sal_Glb_Customer_Mst as D on D.CustomerID =A.CustomerId where A.AutoId = '" + AutoId + "'";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                HidCustomerGroupId.Value = dt.Rows[0]["CustomerGroupId"].ToString();
                txtCustomerGroup.Text = dt.Rows[0]["CustomerGroup"].ToString();
    ...
    protected void FillAllUser(string Searchtext)
    {
        /// <summary>
        /// This method is used to get all users and fill in popup grid.
        /// </summary>
        try
        {
            DataTable dt = new DataTable();

            string sql = @"SELECT U.UserID,U.UserCode,U.UserName,Utype.Type from Com_User_Mst AS U Inner join Com_UserType_Mst Utype On
                           U.UserTypeId=Utype.UserTypeId WHERE U.ActiveStatus =1 AND Utype.Type in ('Customer','C_Owner','AR_Owner') and
                           (U.UserCode like '%" + Searchtext + "%' or U.UserName like '%" + Searchtext + "%' or Utype.Type like '%" + Searchtext + "%')";
    ...
    protected void FillAllCustomerGroup(string Searchtext)
    {
        /// <summary>
        /// This method is used to get all users and fill in popup grid.
        /// </summary>
        try
        {
            ...
            string sql = @"SELECT U.UserID,U.UserName,Utype.Type from Com_User_Mst AS U Inner join Com_UserType_Mst Utype On
                           U.UserTypeId=Utype.UserTypeId WHERE U.ActiveStatus =1 AND Utype.Type in ('Customer') and
                           (U.UserCode like '%" + Searchtext + "%' or U.UserName like '%" + Searchtext + "%' or Utype.Type like '%" + Searchtext + "%')";
